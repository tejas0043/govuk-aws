...
variable "mapit_subnet" {
  type        = "string"
  description = "Name of the subnet to place the mapit instances and EBS volumes"
}
...
module "mapit" {
  ...
  default_tags                  = "${map("Project", var.stackname, "aws_stackname", var.stackname, "aws_environment", var.aws_environment, "aws_migration", "mapit", "aws_hostname", "mapit")}"
  instance_subnet_ids           = "${matchkeys(values(data.terraform_remote_state.infra_networking.private_subnet_names_ids_map), keys(data.terraform_remote_state.infra_networking.private_subnet_names_ids_map), list(var.mapit_subnet))}"
...
resource "aws_ebs_volume" "mapit" {
  availability_zone = "${lookup(data.terraform_remote_state.infra_networking.private_subnet_names_azs_map, var.mapit_subnet)}"
...
resource "aws_iam_role_policy_attachment" "mapit_iam_role_policy_attachment" {
  role       = "${module.mapit.instance_iam_role_name}"
